...
from library.backend import ProgressRecorder
...
class IfcExtractor(object):
    def __init__(self, ifc_file_path: str):
        ...
        self.start = None
        self.config = read_config("../schema/prices.csv")

        # Set the path to your IFC file and weather file TODO dynamically override this based on the user config
        self.weather_file_path = "../schema/AUT_Vienna.Schwechat.110360_IWEC.epw"

        # Open the IFC model from th    e given file path
    ...
    def calculate_facade_area(self, product) -> None:
        # TODO implement algorithm used in voxelisation toolkit
    ...
    def process_products(self) -> None:
        ...
        # ifcopenshell.geom.utils.cache_directory = "/tmp/"
        ...
        try:
            ...
            self.properties["TSA"] = get_plot_area(site)
        ...
        try:
            ...
            for storey in storeys:
                self.properties["BGF"] += get_plot_area(storey)
                if storey.get_info()["Elevation"] == 0:
                    self.properties["BF"] = get_plot_area(storey)
                # pprint(storey.get_info())

        except Exception:
            print(f"Failed to extract floor area.")
        # tasks = [asyncio.create_task(self.calculate_material_cost(product)) for product in products]
        # for task in asyncio.as_completed(tasks):
        #     get task result
        #     result = task.result()
        for i, product in enumerate(products):
            # progress_recorder.set_progress(i + 1, product_quantity, description='Calculating material cost')
